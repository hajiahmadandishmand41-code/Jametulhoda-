...
<section class="admin-welcome">
    <div><p class="admin-kicker">مرکز محتوا</p><h2>همه محتوا</h2><p><?= e(fa_digits((string) $total)) ?> مورد واقعی از پایگاه داده</p></div>
...
<form class="admin-filters" method="get" action="<?= e(url('/admin/content')) ?>">
    ...
    <label>نوع<select name="type"><option value="">همه</option><?php foreach ($typeLabels as $value => $label): ?><option value="<?= e($value) ?>" <?= (($filters['type'] ?? '') === $value ? 'selected' : '') ?>><?= e($label) ?></option><?php endforeach; ?></select></label>
...
<?php if ($pages > 1): ?><nav class="pagination" aria-label="صفحه‌بندی"><?php for ($i = 1; $i <= $pages; $i++): ?><a class="<?= $i === $page ? 'current' : '' ?>" href="<?= e(url('/admin/content?' . http_build_query(array_merge($filters, ['page' => $i])))) ?>"><?= e(fa_digits((string) $i)) ?></a><?php endfor; ?></nav><?php endif; ?>
